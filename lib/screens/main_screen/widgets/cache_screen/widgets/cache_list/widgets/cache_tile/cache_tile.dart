...
class CacheTile extends HookConsumerWidget {
  final Cache cache;

  const CacheTile({Key? key, required this.cache}) : super(key: key);
        ...
        child: Column(
          ...
          children: [
            if (cache.rank <= 3) Row(
              ...
              children: [
                Tooltip(
                  message: cache.rank <= 0
                    ? ''
                    : '${numWithSuffix(cache.rank)} discoverer',
                  child: FaIcon(
              ...
              trailing: IconButton(
                ...
                onPressed: () async {
                  await ref.read(routerProvider.notifier).showBottomSheet(child: Column(
                    ...
                    children: [
                      if (me?.uid != null && cache.resource!.createdBy?.id == me?.uid) ListTile(
                      ...
                      if (me?.uid != null && cache.resource!.createdBy?.id == me?.uid) ListTile(
                        ...
                        onTap: () async {
                          ref.read(routerProvider.notifier).closeBottomSheet();
                          ref.read(routerProvider.notifier).push(route: Routes.resourceEdit, extra: ResourceEditRouteParams(cache: cache));
                        },
                      ),
                      ListTile(
